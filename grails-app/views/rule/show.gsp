<%@ page import="ar.com.burucps.drools.Rule"%>
<!DOCTYPE html>
<html>
<head>
<meta name="layout" content="main">
<g:set var="entityName"
	value="${message(code: 'rule.label', default: 'Rule')}" />
<title><g:message code="default.show.label" args="[entityName]" /></title>
</head>
<body>
	<div id="content" class="clearfix">
		<div class="contentwrapper">
			<div class="heading">
				<h3>
					<g:message code="rule.btnLabel" args="[entityName]" />
				</h3>

				<div class="resBtnSearch">
					<a href="#"><span class="icon16 icomoon-icon-search-3"></span></a>
				</div>

				<ul class="breadcrumb">
					<li>You are here:</li>
					<li><a href="#" class="tip" title="" data-hasqtip="true">
							<span class="icon16 icomoon-icon-screen-2"></span>
					</a> <span class="divider"> <span
							class="icon16 icomoon-icon-arrow-right-3"></span>
					</span></li>
					<li class="active"><g:message code="rule.show.label"
							args="[entityName]" /></li>
				</ul>

			</div>

			<ul class="bigBtnIcon">
				<li><g:link class="create" action="create">
						<span class="icon icomoon-icon-plus"></span>
						<g:message code="default.new.label" args="[entityName]" />
					</g:link></li>
				<li><g:link class="create" action="">
						<span class="icon icomoon-icon-menu"></span>
						<g:message code="default.list.label" args="[entityName]" />
					</g:link></li>
			</ul>

			<div class="row">

				<div class="col-lg-2"></div>

				<div class="col-lg-8">
					<div id="show-rule" class="content scaffold-show" role="main">
						<h1>
							<g:message code="default.show.label" args="[entityName]" />
						</h1>
						<g:if test="${flash.message}">
							<div class="message" role="status">
								${flash.message}
							</div>
						</g:if>
						<div class="panel panel-default">

							<div class="panel-heading">
								<h4>
									<span><g:message code="rule.show.label"
											args="[entityName]" /></span>
								</h4>
							</div>
							<div class="panel-body">

								<ul class="col-lg-12">

									<g:if test="${ruleInstance?.ruleName}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.ruleName.label" default="Rule Name" /></span> <span
											class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:fieldValue bean="${ruleInstance}"
													field="ruleName" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.dialect}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.dialect.label" default="Dialect" /></span> <span
											class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:fieldValue bean="${ruleInstance}"
													field="dialect" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.salience}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.salience.label" default="Salience" /></span> <span
											class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:fieldValue bean="${ruleInstance}"
													field="salience" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.ruleCondition}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.ruleCondition.label" default="Rule Condition" /></span>
											<span class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:fieldValue bean="${ruleInstance}"
													field="ruleCondition" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.ruleConsequence}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.ruleConsequence.label"
													default="Rule Consequence" /></span> <span
											class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:fieldValue bean="${ruleInstance}"
													field="ruleConsequence" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.ruleGroup}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.ruleGroup.label" default="Rule Group" /></span> <span
											class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"> ${g.message(code: fieldValue(bean: ruleInstance, field: "ruleGroup"))}
										</span></li>
									</g:if>

									<g:if test="${ruleInstance?.active}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.active.label" default="Active" /></span> <span
											class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:formatBoolean
													boolean="${ruleInstance?.active}" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.creationDate}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.creationDate.label" default="Creation Date" /></span>
											<span class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:formatDate
													date="${ruleInstance?.creationDate}" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.lastUpdateDate}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.lastUpdateDate.label" default="Last Update Date" /></span>
											<span class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span><span
											class="col-lg-6"><g:formatDate
													date="${ruleInstance?.lastUpdateDate}" /></span></li>
									</g:if>

									<g:if test="${ruleInstance?.ruleSets}">
										<li style="display: block;"><span class="blue col-lg-3"><g:message
													code="rule.ruleSets.label" default="Rule Sets" /></span> <span
											class="icon12 icomoon-icon-arrow-right-5 blue col-lg-1"></span>
											<span class="col-lg-6"><g:each
													in="${ruleInstance.ruleSets}" var="r">
													<g:link controller="ruleSet" action="show" id="${r.id}">
														${r?.encodeAsHTML()}
													</g:link><br/>
												</g:each> </span></li>
									</g:if>
								</ul>

								<ul class="bigBtnIcon" style="display: -webkit-box;">
									<li><g:link class="edit" action="edit"
											resource="${ruleInstance}">
											<span class="icon icomoon-icon-pencil"></span>
											<g:message code="default.button.edit.label" default="Edit" />
										</g:link></li>
								</ul>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</body>
</html>
