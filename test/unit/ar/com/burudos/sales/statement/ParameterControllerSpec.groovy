...
class ParameterControllerSpec extends Specification {
    ...
    void "Test the index action returns the correct model"() {
        ...
        then:"The model is correct"
            !model.parameterInstanceList
            model.parameterInstanceCount == 0
    }
    ...
    void "Test that the show action returns the correct model"() {
        ...
        when:"A domain instance is passed to the show action"
            ...
            controller.show(parameter)

        then:"A model is populated containing the domain instance"
            model.parameterInstance == parameter
    }
    ...
    void "Test the update action performs an update on a valid domain instance"() {
        ...
        when:"A valid domain instance is passed to the update action"
            response.reset()
            populateValidParams(params)
            parameter = new Parameter(params).save(flush: true)
            controller.update(parameter)

        then:"A redirect is issues to the show action"
            response.redirectedUrl == "/parameter/show/$parameter.id"
            flash.message != null
    }
    ...
    void "Test that the delete action deletes an instance if it exists"() {
        ...
        when:"A domain instance is created"
            response.reset()
            populateValidParams(params)
            def parameter = new Parameter(params).save(flush: true)
